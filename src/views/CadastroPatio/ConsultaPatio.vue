    ...
    <v-layout fill-height row justify-center>
      <v-flex md12>
        <v-toolbar flat color="white">
          <v-toolbar-title v-if="!this.$vuetify.breakpoint.xsOnly">Patios</v-toolbar-title>
        ...
        <v-btn class="v-btn v-btn--bottom v-btn--floating v-btn--fixed v-btn--right theme--dark blue darken-1"
               style="width: 65px; height:65px; font-size: 30px" :to="{ name: 'cadastroPatio' }">+</v-btn>
      </v-flex>
      ...
      <v-dialog v-model="deleteDialog" persistent max-width="390">
        <v-card>
          ...
          <v-card-text>
            Deseja realmente excluir o patios "{{selectedItem.nome}}"?
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      async list (params) {
        ...
        try {
          try {
            const result = await patio.listar(params)
      ...
      editItem (item) {
        ...
        this.$router.push({ name: 'cadastroPatio', params: { guid: item.guid } })
      },
      ...
      async deleteItem () {
        ...
        try {
          const result = await patio.deletar(this.selectedItem.guid)
