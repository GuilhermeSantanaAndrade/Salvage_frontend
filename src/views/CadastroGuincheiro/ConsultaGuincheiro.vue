    ...
    <v-layout fill-height row justify-center>
      <v-flex md12>
        <v-toolbar flat color="white">
          <v-toolbar-title v-if="!this.$vuetify.breakpoint.xsOnly">Guincheiros</v-toolbar-title>
        ...
        <v-btn class="v-btn v-btn--bottom v-btn--floating v-btn--fixed v-btn--right theme--dark blue darken-1"
               style="width: 65px; height:65px; font-size: 30px" :to="{ name: 'cadastroGuincheiro' }">+</v-btn>
      </v-flex>
      ...
      <v-dialog v-model="deleteDialog" persistent max-width="390">
        <v-card>
          ...
          <v-card-text>
            Deseja realmente excluir o guincheiro "{{selectedItem.nome}}"?
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      async list (params) {
        ...
        try {
          try {
            const result = await guincheiro.listar(params)
      ...
      editItem (item) {
        ...
        this.$router.push({ name: 'cadastroGuincheiro', params: { guid: item.guid } })
      },
