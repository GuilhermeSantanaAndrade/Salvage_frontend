    ...
    <v-layout fill-height row justify-center>
      <v-flex md12>
        <v-toolbar flat color="white">
          <v-toolbar-title v-if="!this.$vuetify.breakpoint.xsOnly">Despachantes</v-toolbar-title>
        ...
        <v-data-table :headers="headers" :items="items" :search="search" class="elevation-1">
          <template slot="items" slot-scope="props">
            <td class="justify-center layout px-0">
              <v-icon small color="blue darken-2" @click="editItem(props.item)" style="margin-right: 20px; cursor: pointer;">edit</v-icon>
              <v-icon small color="red darken-2" @click="deleteConfirmationMessage(props.item)" style="margin-left: 20px; cursor: pointer;">delete</v-icon>
            </td>
        ...
        <v-btn class="v-btn v-btn--bottom v-btn--floating v-btn--fixed v-btn--right theme--dark blue darken-1"
               style="width: 65px; height:65px; font-size: 30px" :to="{ name: 'cadastroDespachante' }">+</v-btn>
      </v-flex>
      ...
      <v-dialog v-model="deleteDialog" persistent max-width="390">
        <v-card>
          ...
          <v-card-text>
            Deseja realmente excluir o despachante "{{selectedItem.nome}}"?
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      async list (params) {
        ...
        try {
          try {
            const result = await despachante.listar(params)
      ...
      editItem (item) {
        ...
        this.$router.push({ name: 'cadastroDespachante', params: { guid: item.guid } })
      },
      ...
      async deleteItem () {
        ...
        try {
          const result = await despachante.deletar(this.selectedItem.guid)
