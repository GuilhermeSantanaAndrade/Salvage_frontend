      ...
      <v-layout justify-space-between pa-3>
        <v-flex xs5>
          <v-treeview :active.sync="active" :items="items" :load-children="fetchUsers" :open.sync="open" activatable
            ...
            <v-icon v-if="!item.children" slot="prepend" slot-scope="{ item, active }" :color="active ? 'primary' : ''">chevron_right</v-icon>
          </v-treeview>
        </v-flex>
    ...
    computed: {
      items () {
        return [
          ...
          {
            name: 'Alocar Guincheiro',
            children: this.users
          },
          {
            name: 'Aguardando Guincho',
            children: this.users
          },
          {
            name: 'Alocar Despachante',
            children: this.users
          },
          {
